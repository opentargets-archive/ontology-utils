...
def test_access_ini_file():
    assert not Config.ONTOLOGY_CONFIG == None
    assert len(Config.ONTOLOGY_CONFIG.sections()) > 0
    for each_section in Config.ONTOLOGY_CONFIG.sections():
        for (each_key, each_val) in Config.ONTOLOGY_CONFIG.items(each_section):
            print(each_key)
            print(each_val)
    assert not Config.ONTOLOGY_CONFIG.get('uris', 'hpo') == None
